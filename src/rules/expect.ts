...
const lineFromPosition = (pos: number, sourceFile: SourceFile) =>
 ...
 */
function getParserServices(context: TSESLint.RuleContext<any, any>) {
  if (
...
function getNodeForExpectType(node: Node) {
  ...
  ts.forEachChild(sourceFile, function iterate(node) {
    ...
    if (expected !== undefined) {
      ...
      if (node.kind === ts.SyntaxKind.ExpressionStatement) {
        node = (node as ExpressionStatement).expression;
      }
...
module.exports = createRule({
  ...
  create(context) {
    ...
    function getLine(comment: TSESTree.Comment) {
    ...
    function parseAssertions(comments: TSESTree.Comment[]) {
